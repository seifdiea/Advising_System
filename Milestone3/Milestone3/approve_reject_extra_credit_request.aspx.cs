...
{
    public partial class approve_reject_extra_credit_request : System.Web.UI.Page
    {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        if (type.ToLower() != "credit_hours")
                        {
                            DisplayResult("This Page isn't for Evaluating Course Requests! ", "failure");
                            return;
                        }
                    ...
                    command = new SqlCommand("Procedures_AdvisorApproveRejectCHRequest", connection)
                    {
                        CommandType = CommandType.StoredProcedure
                    };
                    command.Parameters.AddWithValue("@requestID", requestId);
                    command.Parameters.AddWithValue("@current_sem_code", currentSemesterCode);
